...
public ResponseEntity<Usuarios> createUser (@RequestBody UserRequest userRequest) {
    System.out.println("Request recibido: " + userRequest);
    System.out.println("Recibido roleId: " + userRequest.getRole_id()); // Esta línea imprime el ID del rol
        

    // Verificar si se ha enviado un rol
    if (userRequest.getRole_id() == null || userRequest.getRole_id() <= 0) {
    ...
    user.setPassword(passwordEncoder.encode(userRequest.getPassword())); // Encriptar contraseña
    user.setEmail(userRequest.getEmail());
    user.setCreatedAt(new Date()); // Establecer la fecha y hora actuales

    // Buscar el rol en la base de datos por su ID
    Long roleId = userRequest.getRole_id();
    Role role = roleRepository.findById(roleId)
            .orElseThrow(() -> new EntityNotFoundException("Rol no encontrado con ID: " + userRequest.getRole_id()));
    user.setRole(role);  
    ...
    // Crear el usuario
     ...
     System.out.println("Usuario creado para frontend: " + createdUser);
